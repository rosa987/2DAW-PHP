<HTML>
<HEAD><TITLE>ALTA DE CLIENTES</TITLE></HEAD>
<style>
</style>
...
<br>
<h1>ALTA DE CLIENTE (comaltacli.php)</h1>
<form  method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
    ...
    <input type="submit" value="Dar alta a CLIENTE" /> 
</form>
...
<?php
 if (!(empty($_POST))) {
    ...
    if ($_SERVER["REQUEST_METHOD"]== "POST"){  //SI QUITO ESTO AUN FUNCIONA... O-O
        ...
        $ciudad = $_POST["ciudad"];
        if(!empty($nif)){  //nif no puede estar vacio
            if(compararNIFs($conn, $nif)==true){ //si ya existe el nombre, no se da de alta
                ...
                nuevo_Cliente($conn, $nif, $nombre, $apellido, $cp, $direccion, $ciudad); //crear new Cliente
                $conn = null;
            } 
        }
